#!/usr/bin/env bash
# Full TTRL-style GRPO: sample 16 responses per prompt for voting, then update on 8.
...
TRAIN_FILE=${TRAIN_FILE:-/data/home/yanghj/data/complementary_labels/medqa_full_options/train.parquet}
...
UPDATE_ROLLOUT_N=${UPDATE_ROLLOUT_N:-8}
...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-qwen2_5_3b_ttrl_vote16_update8}
CHECKPOINT_DIR=${CHECKPOINT_DIR:-"${HOME}/checkpoints/complementary_labels_ttrl_vote16_update8"}
...
REWARD=(
    ...
    reward.reward_manager.name=MedQATTRLRewardManager
    reward.reward_manager.module.path="${PROJECT_DIR}/rewards/medqa_ttrl_reward.py"
    reward.custom_reward_function.path="${PROJECT_DIR}/rewards/medqa_ttrl_reward.py"
    reward.custom_reward_function.name=compute_score
    +reward.custom_reward_function.reward_kwargs.min_vote_count="${MIN_VOTE_COUNT}"
    +reward.custom_reward_function.reward_kwargs.require_format=True
    +reward.custom_reward_function.reward_kwargs.tie_break_seed="${TIE_BREAK_SEED}"
    +reward.custom_reward_function.reward_kwargs.keep_rollouts_per_prompt="${UPDATE_ROLLOUT_N}"
)
